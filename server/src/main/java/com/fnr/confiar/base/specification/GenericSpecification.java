 ...
 * {@link org.springframework.data.jpa.domain.Specification} interface
		...
		switch (operation) {
			...
			case LIKE: {
				...
				if (filterCriteria.isCaseSensitive()) {
					String value = String.format(QueryConstants.LIKE,
							(filterCriteria.getValue() != null) ? String.valueOf(filterCriteria.getValue()) : "");
			...
			case GREATER_THAN: {
				...
				final Path<?> path = resolvePath(root, filterCriteria.getKey());
				return criteriaBuilder.greaterThan(path.as(comparableFilterCriteria.getClazz()),
						comparableFilterCriteria.getValue());
			}

			case GREATER_THAN_OR_EQUAL_TO: {
				final ComparableFilterCriteria<C> comparableFilterCriteria = getComparableFilterCriteria();
				final Path<?> path = resolvePath(root, filterCriteria.getKey());
				return criteriaBuilder.greaterThanOrEqualTo(path.as(comparableFilterCriteria.getClazz()),
						comparableFilterCriteria.getValue());
			}
			...
			case LESS_THAN: {
				final ComparableFilterCriteria<C> comparableFilterCriteria = getComparableFilterCriteria();
				final Path<?> path = resolvePath(root, filterCriteria.getKey());
				return criteriaBuilder.lessThan(path.as(comparableFilterCriteria.getClazz()),
						comparableFilterCriteria.getValue());
			}
			...
			case LESS_THAN_OR_EQUAL_TO: {
				final ComparableFilterCriteria<C> comparableFilterCriteria = getComparableFilterCriteria();
				final Path<?> path = resolvePath(root, filterCriteria.getKey());
				return criteriaBuilder.lessThanOrEqualTo(path.as(comparableFilterCriteria.getClazz()),
						comparableFilterCriteria.getValue());
			}
